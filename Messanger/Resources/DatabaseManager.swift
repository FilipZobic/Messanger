...
extension DatabaseManager {
    ...
    /// Create new conversation with target email
        ...
        ref.observeSingleEvent(of: .value, with: {[weak self] snapshot in
            ...
            if var conversations = userNode["conversations"] as? [[String: Any]] {
                ...
                ref.setValue(userNode, withCompletionBlock: { [weak self] error, _ in
                    ...
                    self?.finishCreatingConversation(name: name, conversationID: convId, firstMessage: firstMessage, completion: completion)
                })
            } else {
    ...
    public func getAllConversations(for email: String, completion: @escaping (Result<[Conversation], Error>) -> Void) {
        database.child("\(email)/conversations").observe(.value, with: {snapshot in
            ...
            let conversations: [Conversation] = value.compactMap({ dictionary in
                ...
                return Conversation(id: conversationId, name: name, recipient: recipient, latestMessage: latestMessageObject)
            })
            print(conversations)
        ...
        database.child("\(id)/messages").observe(.value, with: { snapshot in
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                guard let name = dictionary["name"] as? String,
                      ...
                      let senderEmail = dictionary["sender"] as? String,
                      let dateString = dictionary["date"] as? String,
                      let date = ChatViewController.dateFormatter.date(from: dateString),
                      let type = dictionary["type"] as? String else {
                          print("Failed getting convo")
                          return nil
                      }
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value, with: {
                    ...
                    if var currentUserConversations = snapshot.value as? [[String: Any]]  {
                        ...
                        if var targetConversation = targetConversation {
                            ...
                            let newConversationData: [String: Any] = [
                                "id": conversation,
                                "recipient": DatabaseManager.safeEmail(otherUserEmail),
                                "name": name,
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                            ...
                            if var otherUserConversations = snapshot.value as? [[String: Any]] {
                                ...
                                if var targetConversation = targetConversation {
                                    ...
                                    let newConversationData: [String: Any] = [
                                        "id": conversation,
                                        "recipient": DatabaseManager.safeEmail(currentEmail),
                                        "name": currentName,
                                        "last_message": updatedValue
                                    ]
    ...
    public func conversationExists(with targeRecipientEmail: String, completion: @escaping (Result<String, Error>) -> Void) {
        let safeRecipientEmail = DatabaseManager.safeEmail(targeRecipientEmail)
        ...
        database.child("\(safeRecipientEmail)/conversations").observeSingleEvent(of: .value, with: {snapshot in
            ...
            if let conversation = collection.first(where: {
                guard let targetSenderEmail = $0["recipient"] as? String else {
                    return false
                }
